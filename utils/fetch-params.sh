...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/RavenCashParams"
else
    PARAMS_DIR="$HOME/.ravencash-params"
fi
...
DOWNLOAD_URL="https://github.com/The-RavenCash-Endeavor/ravencash/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with RVH_DISABLE_SOMETHING=1
RVH_DISABLE_WGET="${RVH_DISABLE_WGET:-}"
#RVH_DISABLE_IPFS="${RVH_DISABLE_IPFS:-}"
RVH_DISABLE_CURL="${RVH_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$RVH_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$RVH_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$RVH_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the RavenCash parameters!
...
This script will fetch the RavenCash SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common RavenCash SNARK parameters. Note that it is

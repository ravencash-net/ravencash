...
# ravencash_hash
RUN git clone https://github.com/The-RavenCash-Endeavor/ravencash_hash
RUN cd ravencash_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} ravencash
RUN useradd -u ${USER_ID} -g ravencash -s /bin/bash -m -d /ravencash ravencash
...
RUN mkdir /ravencash-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /ravencash-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /ravencash-src

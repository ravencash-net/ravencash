...
// which are part of the RavenCash repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="ravencash-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /ravencash-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "ravencash-ravencash/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from ravencash-ravencash/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'ravencash-ravencash/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from ravencash-ravencash/develop"
            }
          }

          if (hasCache) {
            sh "cd /ravencash-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /ravencash-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /ravencash-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /ravencash-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /ravencash-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /ravencash-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /ravencash-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }

...
#if defined(NDEBUG)
# error "RavenCash Core cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "RavenCash Signed Message:\n";
                                     ...
                                     const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool fDryRun)
{
    ...
    for (const CTxIn &txin : tx.vin)
    {
        ...
        if (itConflicting != pool.mapNextTx.end())
        {
            ...
            // Transaction conflicts with mempool and RBF doesn't exist in RavenCash
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    if (nPrevHeight == 0) {
        ...
        nSubsidyBase = 3.125;
    } else if (nPrevHeight <= 7000000) {
        nSubsidyBase = 1.5625;
    } else if (nPrevHeight <= 8000000) {
        nSubsidyBase = 0.78125;
    } else if (nPrevHeight <= 9000000) {
        nSubsidyBase = 0.390625;
    } else {
	    nSubsidyBase = 0.01;
    }
...
void UpdateCoins(const CTransaction& tx, CCoinsViewCache& inputs, CTxUndo &txundo, int nHeight, uint256 blockHash, CAssetsCache* assetCache, std::pair<std::string, CBlockAssetUndo>* undoAssetData)
{
    ...
    // add outputs
    AddCoins(inputs, tx, nHeight, blockHash, false, assetCache, undoAssetData); /** RVH START */ /* Pass assetCache into function */ /** RVH END */
}
...
void ThreadScriptCheck() {
    RenameThread("ravencash-scriptch");
...
static int64_t nTimeRavenCashSpecific = 0;
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, CAssetsCache* assetsCache = nullptr, bool fJustCheck = false)
{
    ...
    /// RAVENCASH: Check superblock start
    ...
    // RAVENCASH : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if (sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        for (const auto& tx : block.vtx) {
            ...
            if (llmq::chainLocksHandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {
                ...
                LOCK(cs_main);
                return state.DoS(10, error("ConnectBlock(RAVENCASH): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), conflictLock->txid.ToString()),
                                 REJECT_INVALID, "conflict-tx-lock");
            }
        }
    } else {
        LogPrintf("ConnectBlock(RAVENCASH): spork is off, skipping transaction locking checks\n");
    }
    ...
    // RAVENCASH : MODIFIED TO CHECK SMARTNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, (blockReward + specialTxFees), strError)) {
        return state.DoS(0, error("ConnectBlock(RAVENCASH): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }
    ...
    if (!IsBlockPayeeValid(*block.vtx[0], pindex->nHeight, blockReward, specialTxFees)) {
        return state.DoS(0, error("ConnectBlock(RAVENCASH): couldn't find smartnode or superblock payments"),
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, fJustCheck, fScriptChecks)) {
        return error("ConnectBlock(RAVENCASH): ProcessSpecialTxsInBlock for block %s failed with %s",
    ...
    int64_t nTime5 = GetTimeMicros(); nTimeRavenCashSpecific += nTime5 - nTime4;
    LogPrint(BCLog::BENCHMARK, "    - RavenCash specific: %.2fms [%.2fs]\n", 0.001 * (nTime5 - nTime4), nTimeRavenCashSpecific * 0.000001);
...
static bool RollforwardBlock(const CBlockIndex* pindex, CCoinsViewCache& inputs, const CChainParams& params, CAssetsCache* assetsCache = nullptr)
{
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, false /*fJustCheck*/, false /*fScriptChecks*/)) {
        return error("RollforwardBlock(RVH): ProcessSpecialTxsInBlock for block %s failed with %s",
            pindex->GetBlockHash().ToString(), FormatStateMessage(state));
    }

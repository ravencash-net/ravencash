...
const char * const BITCOIN_CONF_FILENAME = "ravencash.conf";
const char * const BITCOIN_PID_FILENAME = "ravencashd.pid";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\RavenCashCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\RavenCashCore
    // Mac: ~/Library/Application Support/RavenCashCore
    // Unix: ~/.ravencashcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "RavenCashCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/RavenCashCore";
#else
    // Unix
    return pathRet / ".ravencashcore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty ravencash.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override ravencash.conf

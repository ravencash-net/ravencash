...
void PrepareShutdown()
{
    ...
    RenameThread("ravencash-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-litemode", strprintf(_("Disable all RavenCash specific functionality (Smartnodes, PrivateSend, InstantSend, Governance) (0-1, default: %u)"), 0));
    strUsage += HelpMessageOpt("-sporkaddr=<ravencashaddress>", strprintf(_("Override spork address. Only useful for regtest and devnet. Using this on mainnet or testnet will ban you.")));
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/RavenCashChain/RavenCash>";
    const std::string URL_WEBSITE = "<https://ravencash.net>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("ravencash-loadblk");
...
/** Sanity checks
 *  Ensure that RavenCash Core is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("RavenCash Core version %s\n", FormatFullVersion());
}
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single RavenCash Core process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
	...
	GenerateRavenCashs(fGenerate, gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
    ...
    // lite mode disables all RavenCash-specific functionality
    fLiteMode = gArgs.GetBoolArg("-litemode", false);
    LogPrintf("fLiteMode %d\n", fLiteMode);

    if(fLiteMode) {
        InitWarning(_("You are starting in lite mode, most RavenCash-specific functionality is disabled."));
    }

    if((!fLiteMode && fTxIndex == false)
       && chainparams.NetworkIDString() != CBaseChainParams::REGTEST) { // TODO remove this when pruning is fixed. See https://github.com/The-RavenCash-Endeavor/ravencash/pull/1817 and https://github.com/The-RavenCash-Endeavor/ravencash/pull/1743
    ...
    // ********************************************************* Step 10c: schedule RavenCash-specific tasks

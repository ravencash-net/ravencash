...
- [Development guidelines](#development-guidelines)
    - [General RavenCash Core](#general-ravencash-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
RavenCash Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/The-RavenCash-Endeavor/ravencash/blob/master/contrib/valgrind.supp))
...
$ valgrind -v --leak-check=full src/ravencashd -printtoconsole
```
...
RavenCash Core can be compiled with various "sanitizers" enabled, which add
...
- ThreadRPCServer : Remote procedure call handler, listens on port 7961 for connections and services them.
...
However, in open source software such as RavenCash Core, where everyone uses
...
pay attention to for reviewers of RavenCash Core code.

General RavenCash Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: RavenCash Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against RavenCash Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `ravencash-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `ravencash-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of RavenCash Core, the wallet was always
